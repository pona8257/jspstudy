...
public class UploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 	...
		 	2. COS File Upload Library 메뉴에 있는 cos-22.05.zip 파일을 다운로드
		...
		/*
		 	...
		 	3. MultipartRequest 클래스를 이용해서 파일 업로드를 처리할 수 있는데, cos.jar가 MultipartRequest 클래스
		 */
		...
		// 업로드 진행하기 (HttpServletRequest request를 이용해서 MultipartRequest 객체를 생성하면 업로드가 진행된다.)
		...
		// 요청 파라미터(MultipartRequset multipartRequest를 이용해서 파라미터를 가져와야 한다)
		...
		// 첨부 파일 정보
		// 파일명, 경로, 최종 수정일(yyyy-mm-dd), 파일크기(KB)
		...
		out.println("<li><a href=\"/01_Servlet/FileListServlet?parent=" + URLEncoder.encode(parent, "UTF-8") + "\">첨부된 파일 목록 보기</a></li>");		out.println("</ul>");
		out.flush();
		out.close();
