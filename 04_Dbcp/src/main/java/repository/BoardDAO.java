...
public class BoardDAO {
	...
	public List<BoardDTO> selectBoardList() {
		...
		try {
			...
			// 5. PreparedStatement 객체를 이용해 쿼리문 실행(SELECT문 실행은 excuteQuery 메소드로 한다.)
			rs = ps.executeQuery();
			...
			// 예외 발생 여부와 상관 없이 항상 자원의 반납을 해야한다.
			close();
		}
	...
	public BoardDTO selectBoardByNo(int board_no) {
		...
		try {
			...
			// 6. PreparedStatement 객체를 이용해 쿼리문 실행(SELECT문 실행은 excuteQuery 메소드로 한다.)
			rs = ps.executeQuery();
	...
	// 게시글 삽입하기
	public int insertBoard(BoardDTO board) {	// write.jsp에서 넘어온 title, content값이 board에 저장된다
		...
		try {
			...
			// 6. PreparedStatement 객체를 이용해 쿼리문 실행(SELECT문 실행은 excuteUpadate 메소드로 한다.)
	...
	public int updateBoard(BoardDTO board) {
		...
		try {
			...
			// 6. PreparedStatement 객체를 이용해 쿼리문 실행(UPDATE문 실행은 excuteUpadate 메소드로 한다.)
	...
	public int deleteBoard(int board_no) {
		...
		try {
			...
			// 6. PreparedStatement 객체를 이용해 쿼리문 실행(DELETE문 실행은 excuteUpadate 메소드로 한다.)
