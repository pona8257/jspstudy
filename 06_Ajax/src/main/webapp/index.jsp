...
<body>
	...
	<script>
		...
		// 함수 호출(자바스크립트에는 호이스팅 기능이 있어 호출을 먼저하든 나중하든 상관 없다)
		fnInit();
		fnGetAllMember();
		...
		function fnGetAllMember(){
			$.ajax({
				...
				// 응답
				dataType: 'json',
				success: function(resData){	//  detailserver의 응답 데이터는 resData로 전달된다
					/*
					...
					*/
					...
					if(resData.memberCount === 0){
						...
						/* $.each(배열, (인덱스, 요소) => {})*/
						$.each(resData.memberList, function(i ,element){	// element는 한 회원 객체를 의미
							let str = '<tr>';
							...
							str += '<td><input type="button" value="조회" class="btn_detail" onclick="fnGetMember(' + element.memberNo + ')"></td>';	// for문 이여서 id값을 주면 안됨 그래서 class
			...
			$.ajax({
				...
				success: function(resData){	// try문의 응답이 resData에 저장된다. resData = {"insertResult" : 1}
					if(resData.insertResult === 1){
						...
						fnInit();			// 입력란 초기화(성공했으니까 안그럼 계속 남아있음)
		...
		// onclik="fnGetMember(element.memberNo)";
		...
		function fnGetMember(memberNo){
			$.ajax({
				...
				data: 'memberNo=' + memberNo,	// 파라미터 이름 + 값
				// 응답
				dataType: 'json',
				success: function(resData){	
					alert('회원 정보가 조회되었습니다.');
					$('#id').val(resData.member.id).prop('disabled', true);    // resData = {"member":{"memberNo":회원번호, ...}}
					$('#name').val(resData.member.name);
					$(':radio[name=gender][value=' + resData.member.gender + ']').prop('checked', true);		// :radio[name=gender] 얘는 남자, 여자 두개를 지정
